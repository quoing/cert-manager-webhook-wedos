...
func (d *DNSProvider) Present(fqdn, token, challenge string) error {
	ctx := context.Background()

	authZone, err := dns01.FindZoneByFqdn(fqdn)
	if err != nil {
		return fmt.Errorf("wedos: could not determine zone for fqdn %q: %w", fqdn, err)
	}
	...
	if err != nil {
		return fmt.Errorf("wedos: could not get records for FQDN %q: %w", fqdn, err)
	}

	for _, candidate := range records {
		if candidate.Type == "TXT" && candidate.Name == subDomain && candidate.Data == challenge {
	...
	if err != nil {
		return fmt.Errorf("wedos: could not add TXT record for FQDN %q: %w", fqdn, err)
	}

	err = d.client.Commit(ctx, authZone)
	if err != nil {
		return fmt.Errorf("wedos: could not commit TXT record for FQDN %q: %w", fqdn, err)
	}
...
func (d *DNSProvider) CleanUp(fqdn, token, challenge string) error {
	ctx := context.Background()
	...
	for _, candidate := range records {
		...
		if err != nil {
			return fmt.Errorf("wedos: could not remove TXT record for FQDN %q: %w", fqdn, err)
		}
